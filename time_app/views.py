...
# indexのビュー関数_______________________________________________________________________
  ...
  if request.method =='POST':
    ...
    if request.POST['mode'] == '__friends_form__':
      ...
      for item in fds:
        ...
        vlist.append(item.user.username)
      # メッセージを設定
      messages.success(request, 'テェックされたFriendを' + \
        sel_group + 'に登録しました。')
      # フォームの用意
...
def creategroup(request):
  ...
  return redirect(to='/time_app/groups')
...
def post(request):
  ...
  return render(request, 'time_app/post.html', params)
